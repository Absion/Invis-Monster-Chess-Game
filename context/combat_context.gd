...
func _draw_visual_grid() -> void:
	...
	# Pass the base materials to GridManager so it can restore them without allocating new materials
	grid_manager.base_white_mat = white_mat
	grid_manager.base_black_mat = black_mat
	...
	for x in range(GridManager.GRID_SIZE_X):
		for z in range(GridManager.GRID_SIZE_Z):
			...
			cell.mesh = box_mesh
			# ⚡ Bolt Optimization: Use shared materials to preserve batching instead of duplicating
			if (x + z) % 2 == 0:
