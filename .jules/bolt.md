...
**Learning:** Accessing Object properties (e.g., `actor.grid_x`) repeatedly inside deeply nested loops is slow due to GDScript VM overhead and dynamic lookups. The `GirlAIController` accessed object properties of all monsters for every single reachable grid cell, creating an O(R^2 * M) overhead.
