...
## 2026-06-27 - Dynamic Array Allocation for Fixed Structures inside Methods
**Learning:** Dynamically allocating and calculating arrays of fixed values (e.g., relative grid coordinate offsets like `dirs = []` for AOE attacks) within methods at runtime causes unnecessary CPU overhead and generates excessive Garbage Collection (GC) pressure.
**Action:** Pre-calculate these fixed structures and store them in a `const` Array at the class level. Replace dynamic loops and `append` operations with these pre-computed constants.
