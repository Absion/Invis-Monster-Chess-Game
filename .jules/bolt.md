...
## 2026-06-27 - 3D Node Transform Updates on MouseMotion
**Learning:** Continuously updating a 3D Node's transform (e.g., `position`) on high-frequency events like `InputEventMouseMotion` triggers unnecessary rendering server invalidations.
**Action:** Cache logical states (e.g., grid cell coordinates) and only update 3D transforms when the logical boundary is crossed.
